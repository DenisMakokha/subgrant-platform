-- Migration: add indexes and materialized view refresh helpers (no transaction)
-- Path: api/scripts/migrations/2025Q1_ssot_cutover/add_performance_indexes_no_tx.sql
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_grants_overview') THEN
    EXECUTE 'REFRESH MATERIALIZED VIEW CONCURRENTLY mv_grants_overview';
  END IF;
END;
$$;
