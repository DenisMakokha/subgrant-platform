...
async function runContractSSOTMigrations(shouldClosePool = true) {
  try {
    logger.info('Running Contract SSOT migrations...');
    ...
    logger.info('All Contract SSOT migrations completed successfully');
  } catch (error) {
    logger.error('Error running Contract SSOT migrations:', error);
    process.exit(1);
  } finally {
